...
import { enrichBook } from './data/editorial.js';
...
const enrichedBooks = books.map(enrichBook);
...
const button = b => `<button class="book-card" data-book="${b.slug}" style="--accent:${b.accent}" aria-label="Xem ${title(b)}"><span class="cover-stage">${b.cover ? image(b) : '<span class="art-type"><small>LITTLE HISTORIES</small><span>Art</span><small>CHARLOTTE MULLINS</small></span>'}<span class="cover-arrow" aria-hidden="true">↗</span></span><span class="book-meta">${b.topicTags[0]}${b.status === 'forthcoming' ? ' · Sắp ra mắt' : ''}</span><h3>${title(b)}</h3><span class="author">${b.author}</span><p>${b.lede || b.hook}</p></button>`;
...
function renderBooks(topic = -1) {
  const visible = enrichedBooks.filter(b => topic === -1 || b.primaryTopic === topic);
  grid.innerHTML = visible.map(button).join('');
  document.querySelector('#result-count').textContent = topic === -1 ? '' : `${visible.length} cuốn cho mối tò mò của bạn`;
}
...
document.querySelector('#hero-art').innerHTML = heroBooks.map((slug,i) => {const b=enrichedBooks.find(x=>x.slug===slug);return `<button class="hero-cover hero-cover-${i}" data-book="${slug}" aria-label="Xem ${title(b)}">${image(b,i<3)}</button>`;}).join('');
document.querySelector('#published-count').textContent = enrichedBooks.filter(b => b.status === 'published').length;
document.querySelector('#forthcoming-count').textContent = enrichedBooks.filter(b => b.status === 'forthcoming').length;
document.querySelector('#authors').innerHTML = enrichedBooks.map(b=>`<button data-book="${b.slug}" class="author-card" style="--accent:${b.accent}"><img src="${b.authorProfile.portrait}" alt="Chân dung minh họa ${b.author}" width="96" height="96"><span class="author-info"><strong>${b.author}</strong><small>${b.authorProfile.field}</small><em>${b.authorProfile.shortCredential}</em></span><span aria-hidden="true">↗</span></button>`).join('');
const art=enrichedBooks.find(b=>b.status==='forthcoming');
...
function openBook(slug, trigger) {
  const b = enrichedBooks.find(x => x.slug === slug);
  if (!b) return;
...
const structured=document.createElement('script');structured.type='application/ld+json';structured.textContent=JSON.stringify({'@context':'https://schema.org','@type':'ItemList',name:'Lược sử · Nhã Nam',numberOfItems:enrichedBooks.length,itemListElement:enrichedBooks.map((b,i)=>({'@type':'ListItem',position:i+1,item:{'@type':'Book',name:title(b),author:{'@type':'Person',name:b.author}}}))});document.head.append(structured);
